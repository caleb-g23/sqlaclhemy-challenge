...
# function that returns the the date one year from the most recent date
...
def date_prev_year():
    ...
    first_date = dt.datetime.strptime(most_recent_date, "%Y-%m-%d")
...
def homepage():
    ...
    <strong>/api/v1.0/precipitation</strong> </li>
    <li><a href = "/api/v1.0/stations"> Stations </a>:
    <strong>/api/v1.0/stations</strong></li>
    <li><a href = "/api/v1.0/tobs"> TOBS </a>: <strong>/api/v1.0/tobs</strong>
    </li>
    <li>To retrieve the minimum, average, and maximum temperatures for a
    specific start date, use <strong>/api/v1.0/&ltstart&gt</strong>
    (replace start date in yyyy-mm-dd format)</li>
    <li>To retrieve the minimum, average, and maximum
    temperatures for a specific
    start-end range, use <strong>/api/v1.0/&ltstart&gt/&ltend&gt</strong>
    (replace start and end date in yyyy-mm-dd format)</li>
    </ul>
    """
...
def precipitation():
    ...
    # Query precipitation data from last 12 months from the most
    # recent date from Measurement table
    prcp_data = (
...
# Define what to do when the user hits the URL with a specific start date or
# start-end range
...
def cal_temp(start=None, end=None):
    ...
    if end is None:
        ...
        startdata = session.query(*sel).filter(Measurement.date >= start).all()
        # Convert list of tuples into normal list
        start_list = list(np.ravel(startdata))

        # Return a list of jsonified minimum, average and maximum
        # temperatures for a specific start date
